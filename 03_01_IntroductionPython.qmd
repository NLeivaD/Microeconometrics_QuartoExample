...
library(reticulate)
Sys.setenv(RETICULATE_PYTHON = "C:/Users/Usuario/AppData/Local/Programs/Python/Python311/python.exe")
use_python("C:/Users/Usuario/AppData/Local/Programs/Python/Python311/python.exe")
repl_python()
```

# Preliminaries
We will assume that when you are reading this you have downloaded Python and have been able to access Python in some way. This may be via the command line, but most likely we imagine you will have some GUI which allows you to see a number of windows including a space for graphical output, a description of the contents of your Python Environment (for example any data you have loaded), potentially a place to write your code in Python, and, most importantly, a "shell", or Python interface, which looks like the following:
...
Here, Python code can be entered directly following a prompt which is the `>>>` symbol at the bottom of the window above. From the moment you open Python you are in a Python session until the moment you close it, from now on understand a session as this time between you open and close Python. The most simple way to execute code in Python is to simply input commands into your Python prompt  So, for example, you could write the following at to the right of the `>>>` symbol, and Python will evaluate this code:
...
One final thing to note from this Python script is that the output from the final line of code (`4+4`) was not printed in the console. In Python, when you type `4+4` at the command line, you get immediate output because Python is operating in an interactive mode. In interactive mode, Python evaluates expressions as soon as you enter them and displays the result to the console. However, when you write `4+4` in a script and then run the script, there is no automatic output to the console.  However, it is important to note that the calculation *is* conducted. You can confirm this yourself if you'd like by explicitly requesting that Python provide this output when it runs the code, replacing `4+4` in the code with `print(4+4)`.
