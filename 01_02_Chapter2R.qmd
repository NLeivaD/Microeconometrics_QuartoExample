...
For an introduction to the bootstrap, refer to Section XX of the book.  Here we will examine a computational implementation of a bootstrap standard error and confidence intervals, using data from @Banerjeeetal2021 which we have worked with above. First we will consider the main regression in Code Call-Out 2.1 $$Y_i = \mu + \tau_{ATE}W_i + \varepsilon_i.$$  Here we will work with the data we have loaded previously as data, and the condition `el1==1` which is XXXXX, and examine the asset index outcome (`asset_ind_tot_el1`).
...
There is more than one way to make bootstrap, within this ways we can find parametric and non-parametric methods. In this example we show a non-parametric bootstrap. The first step is to sample with replacement the ordered pairs  $\left\{(y_i,w_i)\right\}_{i=1}^{N} = \left\{(y_1,w_1),\ldots,(y_N,w_N)\right\}$ from the original regression, obtaining $N$ resampled pairs $\left\{(y_i^*,w_i^*)\right\}_{i}^{N} = \left\{(y_1^*,w_1^*),\ldots,(y_N^*,w_N^*)\right\}$. In order to do this firs we sample with replacement numbers from 1 to the number of rows from the dataset used to estimate the model
...
With this we have the rows that are sampled and we can build our artificial sample named bootstrap sample
...
From this estimation we have obtained $\widehat{\tau}_{ATE}^* = -0.0602$ with a standard error of 0.0386 and a p-value of 0.12, much closer to be significative at conventional levels than $\widehat{\tau}_{ATE}$. The next step is to repeat the previous steps $B$ times and get $B$ estimates of $\widehat{\tau}_{ATE,(b)}^*$ with $b\in\{1,\ldots,B\}$
...
As there can be seen in the empirical distribution the distribution is almost centered at 0 with the greater density around 0 so is reasonable to not reject the null hypothesis of $\tau = 0$. With $\left\{\widehat{\tau}^{(b)}\right\}_{b=1}^{B}$ also can be computed an empirical variance (and standard error) or confidence interval based on empirical quantiles as follows

```{r}
var_tau <- var(taus)
print(paste0("The variance estimates is: ", round(var_tau, 4), 
             " in consequence the standard error estimates is: ", 
...
This limits of the interval are shown with the red dashed lines in the empirical distribution and as seen in the empirical 95% confidence interval zero is included.
