...
As you can see, `R` has understand the instruction that you want to add 4 plus 4 and show the result, which is output directly below the code in the `R` console.  Later in this chapter we will properly explain these mathematical operations and the different `R` data types. Another example of `R` execution is that if you type in your console:
