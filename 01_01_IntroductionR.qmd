...
as.numeric(c(1,"A"))
```
In the above, the inclusion of a string which cannot be logically converted to a number implies that `R` has issued a warning and converted this element to `NA`.  `NA` is `R`'s Not available or missing value indentifier.  We will discuss a bit later on about how to deal with this in code. 
