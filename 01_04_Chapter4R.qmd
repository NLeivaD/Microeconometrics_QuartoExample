...
Check if we need to introduce sapply here?

# Code Call-outs

## Code Call-out 4.1: Wild cluster bootstrap implementation

To show the differences between the wild cluster bootstrap described in Section 4.2.3.2 of the book and the other clustering options such as standard cluster bootstrap or clustered standard errors we use the data worked by @PorterSerra2020 who conducted a field experiment aimed at increasing tne percentage of woman majoring in economics by exposing students enrolled in introductory classes to successful and charismatic women who majored in economics at the same university. The dataset is provided under the name `PorterSerra2020.csv`

```{r}
data <- read.csv2("../../Ejemplo_QuartoBook/Datasets/PorterSerra2020.csv")
```

Following @PorterSerra2020 we estimate the next Linear Probability Model (LPM) $$y_{i} = \beta_0 + \beta_1 dt_i + \beta_2 dT_i + \beta_3 dt_i \times dT_i + \delta \mathbf{X}_i + \varepsilon_i$$ where $Y$ is a student's decision to major in economics (`econmajor`), i.e., takes one if the student decide to major in economics and zero otherwise, $dt$ (`yr_2016`) a dummy equal to one if she took the class in 2016 and zero if she took a class in 2015, and $dT$ (`treatment_class`) is a dummy equal to one if she is in a treatment class, and zero if she is in a control class. The interaction between these two dummies (`treat2016`) is the coefficient of interest and $\mathbf{X}$ a vector of individual, demographic and class controls such as if the course was taught by a female professor (`female_prof`), if the student is a in-state student (`instate`), if the student is in freshman year (`freshman`), if the student is american (`american`), the cumulative GPA (`ACumGPA`), the student's grade in Principles of Economics (`gradePrinciples`) and if the student take a class with a limit of 40 students (`small_class`).  

The authors identify that as the treatment is at the class level (`class_fe2`), they cluster the standard errors at the class level, but since there are fewer than 30 clusters (12 classes), they apply the correction for the small number of clusters by using wild cluster bootstrapping. Next we follow the algorithm described in the book to perform the wild cluster boostrap.  

First we estimate the LPM in order to get an estimate of the interest coefficient $\widehat{\beta}_3$ along with the cluster-robust standard error $se \left( \widehat{\beta}_3 \right)$ and the $t$-statistic value $\left( \widehat{\beta}_3 - 0 \right) / se \left( \widehat{\beta}_3 \right)$ by standard OLS using the `felm` function from the `lfe` package. The usage of this function includes a four-part formula where the first part is a conventional formula as in `lm`, the second part is the variables that determine the fixed effects, the third part is for an IV formula and the last part is a variable to use if clustered standard errors are desired. As we are not estimating with fixed effects or IV we indicate 0 in the second and third portion of the formula
...
It can be seen that the coefficient of interest is 0.080141 with a cluster standard error of 0.036483 and a statistic of 2.197. This values are stored along with the residuals of the regression $\widehat{\varepsilon}$
...
Second we impose the restriction $\beta_3 = 0$ and reestimate the LPM model to obtain the restricted residuals $\tilde{\varepsilon}$
...
Third we should follow a few steps a large number $B$ times that create artifical bootstrap samples depending on residuals and clusters, so before we generate a common data frame to structure this artificials samples and a data frame to store the results of the bootstrap replications
...
bsample <- data |> filter(female == 1) |>
  select(econmajor, yr_2016, treatment_class, treat2016, female_prof,
         instate, freshman, american, ACumGPA, gradePrinciples,
         small_class, class_fe2)
bsample$eps_tilde = as.numeric(eps_tilde)
```

Now we start with the steps, we show one iteration example and then automate the other times in a loop. For each cluster $g$ assign $a_g = 1$ or $a_g = -1$ with probability 0.5 
...
Now generate the new set of bootstrap errors

```{r}
bsample <- bsample |> mutate(berrors = eps_tilde * ag)
```

Next form the artificial outcome variable
...
Now we estimate the non restricted LPM with this sample
...
Finally we rescue the relevant values
...
And now we repeat this for the other $B-1$ times in a loop
...
```
Finally we also compute the the p-value of our hypothesis
...
As we can see we can't reject the null. We also could repeat this exercise with the `bootest` function from the `fwildclusterboot` developed by @FischerRoodman2021 and get the same conclusion
...
boot <- boottest(object = LPM,
                 ...
                 param = "treat2016",
                 clustid = "class_fe2")
t97_5 <- quantile(boot$t_boot, 0.975)
if(abs(t_beta3_hat) > abs(t97_5)){
  print("The null is rejected.")
} else {
  print("We can't reject the null.")
...
In this case the 97.5% quantile may be different from the manual exercise due to the randomization of the exercise but our conclusion doesn't change as seen with the p-value. Finally the authors in @Roodmanetal2019 mentioned that compute the confidence set require a more complex exercise on obtaining more bootstrap samples but conveniently the user-written function does this efficiently and automatically so we can directly from the results get the confidence interval
...
Finally as a comparative exercise we can compare this results with the ones from the clustered standard errors computed at the beginning where null were rejected and with a standard cluster bootstrap using the package `ClusterBootstrap`
...
In this procedure the null is rejected as 0 is included in the 95% confidence interval.
