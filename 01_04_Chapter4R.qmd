...
To understand the equivalence between the panel event study model described in Section 4.4.2.1 of the book and the "Interaction-weighted (IW) estimator" proposed by @AbrahamSun2018 we follow @ClarkeTapia2020 that works with data from @StevensonWolfers2006 of the no-default divorce reforms (`X_nfd`) and female suicide (`asmrs`) in United States for 49 states (`stfips`) from 1964 to 1996

```{r}
data <- read.csv(file = "../Ejemplo_QuartoBook/Datasets/StevensonWolfers2006.csv")
head(data)
```

In order to prepare our dataset we note that the variable `X_nfd` ($Event_s$) define the year $t$ when no-fault divorce reform took place for each state $s$, with this variable we define `timeToTreat` as the difference between the year $t$ and $Event_s$
...
This new values are leads (previous to treatment) and lags (posterior to treatment).

### Panel Event Study Model

First we estimate a panel event study model following the next equation $$asmrs_{st} = \alpha + \sum_{j=2}^{J} \beta_j (Lead \ j)_{st} + \sum_{k = 0}^{K} \gamma_{k} (Lag \ k)_{st} + \mu_s + \lambda_t + X_{st}^\prime \Gamma + \varepsilon_{st}$$ Where $asmrs_{st}$ is the female suicide rate for all women of state $s$ at period $t$, $(Lead \ j)_{st}$ a dummy variable that takes 1 if the state $s$ at period $t$ is $j$ periods pre-treatment, $(Lag \ k)_{st}$ a dummy variable that takes 1 if the state $s$ at period $t$ is $k$ periods post-treatment, $\mu_s$ and $\lambda_t$ are state and time fixed effects respectively and $X^\prime_{st}$ a vector of covariates for state $s$ at period $t$ such as per-capita income $pcinc_{st}$, homicide mortality $asmrh_{st}$ and the aid to families with dependent children (AFDC) rate for a family of four $cases_{st}$. 

As seen in the equation a binary variable must be included for each lead and lag in our dataset, if we inspect the values

```{r}
unique(data$timeToTreat)
```

We can see how there is $J = 21$ binary leads variables and $K = 27$ lags variable to include. A natural option seems to convert the numeric variable `timeToTreat` into a factor variable and use `C()` in the `lm` function when estimate OLS procedure in order to `R` automatically create the binary variables, but a problem arises when following this steps procedure because of the missing values in `timeToTreat` from those states that never receive treatment. This missing values will be treated as so and when `R` automatically creates the binary variables will consider also missing value in those variables for those states without treatment.  

A solution is to manually create all the binary variables and consider a value of 0 for those states that never receive treatment:
...
Next we estimate by standard OLS procedure using `felm` function from the `lfe` package, note that we omit `Lead1` as a reference base level. The usage of this function include a four-part formula where the first one is a conventional formula as in `lm`, the second part is our variables that determine the fixed effects, the third part is for an IV formula and the last one is a variable to use as cluster in order to compute clustered standard errors. Note the 0 in the IV part of the formula as we don't use instrumental variables in this example and the usage of `cmethod = "reghdfe"` in order to match Stata's `reghdfe` package's cluster-robust standard errors.
...
In the results can be seen how the estimate is the same as in @ClarkeTapia2020. Now we plot the traditional event study plot in order to visualize easier our results
...
3. Estimate the IW estimator following equation \eqref{eq:IWEstimator}.  

In our example $g = \{-21 , -20 , \cdots , 27\}$ so we consider $\ell \in g$, $e \in \{1969 , 1970 , 1971 , 1972 , 1973 , 1974 , 1975 , 1976 , 1977 , 1980 , 1984 , 1985\}$^[You can proof this easily with the use of `sort(unique(data$X_nfd))`.]. Now we build the indicator variables for $E_i$

```{r}
for (i in sort(unique(data$X_nfd))) {
  data[, paste0('E_', i)] <- ifelse((!is.na(data$X_nfd)) & (data$X_nfd == i), 
                                    1, 0)
}
```

With this setup it is possible to notice that from $e$ we have 12 indicator variables and from $g$ we have 48 indicator variables^[Excluding $\ell = -1$.], so from $\displaystyle\sum_{e\neq C}\sum_{\ell\neq-1}\delta_{e, \ell} (\mathbf{1}\{ E_i = e\} \cdot D_{i,t}^{\ell})$ we have 576 indicator variables. To ilustrate this we build three matrix: (i) Dummies for $g = \{-21 , -20 , \cdots , 27\}$, (ii) dummies for $e \in \{1969 , 1970 , 1971 , 1972 , 1973 , 1974 , 1975 , 1976 , 1977 , 1980 , 1984 , 1985\}$ and (iii) dummies for $CATT_{e,\ell}$, also we rename the column names of the last matrix in order to make it clearer

```{r}
library(dplyr)
dummies_timeToTreat <- as.matrix(select(data, contains(c("Lead", "Lag")),
                                        -Lead1))
dummies_Cohort <- as.matrix(select(data, contains("E_")))
for(i in 1:ncol(dummies_Cohort)){
  if(i == 1){
    dummies_CATT <- dummies_timeToTreat * dummies_Cohort[, i]
  } else {
    dummies_CATT <- cbind(dummies_CATT, dummies_timeToTreat * dummies_Cohort[, i])
  }
}
aux = 0
for (g in sort(unique(data$X_nfd))) {
  aux = aux + 1
  cnames <- colnames(dummies_CATT)[(48*(aux-1)+1):(48*aux)]
  colnames(dummies_CATT)[(48*(aux-1)+1):(48*aux)] <- paste0("E", g, "_", cnames)
}
```

Note that some of this indicator variables doesn't have sense as there isn't observations that takes value of 1 and will be a constant of value 0, this means that there are cohorts that never experience the $\ell$ relative period to treatment. As an example there is no observations with non-negative relative periods (lags: $\ell \geq 0$) before or during the first treatment period (1969) and there is no observations with negative relative periods (leads: $\ell < 0$) during or after the last treatment period (1985). In order to delete this non-sense indicator variables we correct the building of the matrix `dummies_CATT` erasing those columns with 0 mean
...
As seen we have reduce the dimensionality of the indicator variables $\displaystyle\sum_{e\neq C}\sum_{\ell\neq-1}\delta_{e, \ell} (\mathbf{1}\{ E_i = e\} \cdot D_{i,t}^{\ell})$ from 576 to 384. Now we can estimate $\delta_{e,\ell}$
...
As there is too many estimates we don't show the full summary, instead we show the first 10 estimates for $CATT_{e,\ell}$

```{r}
CATT_el$coefficients[1:10,]
```

In order to present this coefficient we do as Table 3 of @AbrahamSun2018, we set $\ell$ values in rows and $e$ values in columns so each cell in the $(e,\ell)$ coordinate will be the $\widehat{\delta}$ estimate for cohort $e$ at relative time to treatment $\ell$
...
# Assign each delta_{e,l}
## Auxiliary column indicator
column <- 0
## For each cohort
for (e in sort(unique(data$X_nfd))) {
  ## Add one to column indicator
  column = column + 1
  ## Auxiliary row indicator
  row = 0
  ## For each relative time to treatment
  for (l in -21:27) {
    ## Add one to row indicator
    row = row + 1
    ## Chechk if is base scenario l = -1
    if(l == -1){
      ## Assign 0
      deltas[row, column] = 0
      ## If not, check if is Lead period
    } else if(l < -1){
      ## Define the delta_{e,l} to search
      Catt_searched <- paste0("dummies_CATTE", e, "_Lead", abs(l))
      ## Check if exists with try, if the class of try is "try-error" it doesn't exist
      if(class(try(CATT_el$coefficients[Catt_searched,1], silent = T)) == "try-error"){
        ## Then assign a missing value (NA)
        deltas[row, column] <- NA
        ## If the class isn't "try-error" it exist
      } else{
        ## Then assign the value
        deltas[row, column] <- CATT_el$coefficients[Catt_searched,1]
      }
      ## If isn't a Lead period it is a Lag period
    } else {
      ## Define the delta_{e,l} to search
...
Now we get the the weights of each cohort at the respective relative period. In order to do this we got two ways:  
1. Regress each cohort indicator variable $\mathbf{1} \{ E_i = e \}$ on all the relative period indicator variables $D^\ell_{i,t}$. This method is preferred as we need the variance-covariance matrix of the weights that result from this regressions later in order to build the $\widehat{v}_\ell$ variance. 
2. Get sample share of units in each cohort that is present in the relative period $\ell$.  

First we estimate the mentioned regressions:
...
for (e in sort(unique(data$X_nfd))) {
  ...
  # Note those with 0 value really are relative periods that the cohort doesn't
  # experiment so can be assignad as missing (NA) in order to follow Sun and
  # Abraham (2021) Table 3
...
for (e in sort(unique(data$X_nfd))) {
  ...
  for (l in -21:27) {
    ...
    if(l == -1){
      ...
      ## Check if exists, if has length 0 it doesn't exist
      if(length(Ns$Ne[W_searched]) == 0){
        ## Then assign a missing value (NA)
        units[row, column] <- NA
        ## If the length isn't 0 it exist
      } else{
        ## Then assign the value
        units[row, column] <- Ns$Ne[W_searched]
      }
      ## If isn't a Lead period it is a Lag period
    } else {
      ## Define the weigth to search
      W_searched <- Ns$l == l & Ns$e == e & !is.na(Ns$l) & !is.na(Ns$e)
      ## Check if exists, if has length 0 it doesn't exist
      if(length(Ns$Ne[W_searched]) == 0){
        ## Then assign a missing value (NA)
...
Next we generate $\widehat{v}_{\ell}$ for $\ell = -21, \ldots, 27$ and compare with the results of the Panel Event Study Model
...
As seen almost every coefficient keep their sign, except for a few cases that weren't statiscally significant in the Panel Event Study Model.  

Now we get the standard error for the IW estimators in order to build the confidence interval and compare graphically with the previous estimates. Following @AbrahamSun2018 we got that the variance of $\widehat{v}_\ell$ come from^[This use the next property: Let $X,Y$ be random variables and $a,b\in\mathbb{R}$, then $V(aX \pm bY) = a^2V(X) + b^2V(Y) \pm 2abCov(X,Y)$.]
\begin{align*}
  V \left( \widehat{v}_{\ell} \right) & = V \left( \sum_{e \in h^\ell} \widehat{Pr} \{ E_i = e | E_i \in h^\ell \} \cdot \widehat{\delta}_{e,\ell} \right) \\
  & = \sum_{e \in h^\ell} \left( \widehat{Pr}\{ E_i = e | e \in h^\ell \} \right)^2 V \left( \widehat{\delta}_{e,\ell} \right) + \sum_{e_j \in h^\ell} \sum_{e_k \in h^\ell , e_k \neq e_j} 2 \widehat{Pr}\{ E_i = e_j | e_j \in h^\ell \} \widehat{Pr}\{ E_i = e_k | e_k \in h^\ell \} Cov \left( \widehat{\delta}_{e_j,\ell} , \widehat{\delta}_{e_k,\ell} \right) \\
  & + \sum_{e \in h^\ell} \left( \widehat{\delta}_{e,\ell} \right)^2 V \left( \widehat{Pr}\{ E_i = e | e \in h^\ell \} \right) + \sum_{e_j \in h^\ell} \sum_{e_k \in h^\ell , e_k \neq e_j} 2 \widehat{\delta}_{e_j,\ell}  \widehat{\delta}_{e_k,\ell} Cov \left( \widehat{Pr}\{ E_i = e_j | e_j \in h^\ell \} , \widehat{Pr}\{ E_i = e_k | e_k \in h^\ell \} \right)
\end{align*}
...
We could estimate this manually, and indeed, with the use of seemingly unrelated regression techniques it is possible to estimate the variance of $\widehat{Pr}(\cdot)$ and $\widehat\delta_{e\ell}$ in a single step.  However, in practice we will likely prefer to use a canned routine which allows for the estimation of the interaction weighted estimator, as well as the corresponding variance-covariance matrix. To see that our process of "manually" buidling up Sun and Abraham's estimator from its composite parts, and to additionally conduct inference in a direct way, we can use estimation routines such as `feols` and `sunab` functions from the `fixest` package in `R`. Below we do this, showing the point estimates recovered are identical to what we have done.  

Note the usage of the `sunab` function that takes first the variable that points the cohort and next one of the variable that points the year or the variable that points the relative time to treatment, if the second variable points the year this function automatically and internally creates the relative time to period. Also you can note that we have slightly modify the data for those units never treated take a really high value in the `X_nfd` variable, this is because the `sunab` function would drop this observations if the `X_nfd` has missing values (*NA*) in this variable and instead consider a never treated unit if the relative time to treatment is always negative (for this reason the really high value at `X_nfd` so the relative time will always be negative). Finally note the second part of the formula, after `|`, that points the fixed effects variables and automatically cluster by the first variable
